...
class BaseModel(object):
  ...
  def __init__(self,
    ...
    # Saver
    self.saver = tf.train.Saver(  # source
    # self.saver = tf.compat.v1.train.Saver( # Thanh sua
    #     tf.compat.v1.global_variables(), max_to_keep=hparams.num_keep_ckpts) # Thanh sua
        tf.global_variables(), max_to_keep=hparams.num_keep_ckpts) # source
  ...
  def _set_train_or_infer(self, res, reverse_target_vocab_table, hparams):
    ...
    if self.mode == tf.contrib.learn.ModeKeys.TRAIN:
      ...
      self.sample_words = reverse_target_vocab_table.lookup(
          # tf.cast(self.sample_id)) #Thanh sua
          tf.to_int64(self.sample_id)) # source
    ...
    # params = tf.compat.v1.trainable_variables() # Thanh sua
    params = tf.trainable_variables() # source
  ...
  def build_graph(self, hparams, scope=None):
    ...
    with tf.variable_scope(scope or "dynamic_seq2seq", dtype=self.dtype): #source
    # with tf.compat.v1.variable_scope(scope or "dynamic_seq2seq", dtype=self.dtype): # Thanh sua
      # Encoder
  ...
  def _get_infer_maximum_iterations(self, hparams, source_sequence_length):
    ...
    if hparams.tgt_max_len_infer:
      ...
      # maximum_iterations = tf.cast(tf.round( #Thanh sua
      maximum_iterations = tf.to_int32(tf.round( # source
          # tf.cast(max_encoder_length) * decoding_length_factor)) # Thanh sua
          tf.to_float(max_encoder_length) * decoding_length_factor))  #source
...
class Model(BaseModel):
  ...
  def _build_encoder_from_sequence(self, hparams, sequence, sequence_length):
    ...
    with tf.variable_scope("encoder") as scope: #source
    # with tf.compat.v1.variable_scope("encoder") as scope: # Thanh sua
      dtype = scope.dtype
  ...
  def _build_bidirectional_rnn(self, inputs, sequence_length,
    ...
    bi_outputs, bi_state = tf.nn.bidirectional_dynamic_rnn( # source
    # bi_outputs, bi_state = tf.keras.layers.Bidirectional(keras.layers.RNN(          # Thanh sua keras.layers.Bidirectional(keras.layers.RNN(cell))
        fw_cell,
        bw_cell,
        inputs,
        dtype=dtype,
