...
def _single_cell(unit_type, num_units, forget_bias, dropout, mode,
  ...
  if unit_type == "lstm":
    ...
    # single_cell = tf.keras.layers.LSTMCell(                            #Thanh sua
    #     num_units)
    single_cell = tf.contrib.rnn.BasicLSTMCell(   #source
        num_units,
...
def create_rnn_cell(unit_type, num_units, num_layers, num_residual_layers,
  ...
  if len(cell_list) == 1:  # Single layer.
    ...
    # return tf.keras.layers.StackedRNNCells(cell_list) # Thanh sua
    return tf.contrib.rnn.MultiRNNCell(cell_list) # Source
