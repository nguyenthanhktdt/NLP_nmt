...
def run_main(infer_data, FLAGS, default_hparams, inference_fn, train_fn, target_session=""):
  """Run main."""
  # Job
  jobid = FLAGS.jobid
  ...
  if FLAGS.inference_list:
    ...
    # Inference ###################################
  ckpt = FLAGS.ckpt
  inferred = inference_fn(ckpt, infer_data, hparams, num_workers, jobid)
  return inferred
...
def main(infer_data, FLAGS):
  default_hparams = create_hparams(FLAGS)
  train_fn = train.train
  inference_fn = inference.inference
  return run_main(infer_data, FLAGS, default_hparams, inference_fn, train_fn, target_session="")
...
# print('imported nmt')
def run_predict(check_point, infer_data, vocab_file):
    nmt_parser = argparse.ArgumentParser()
    add_arguments(nmt_parser)
    FLAGS, unparsed = nmt_parser.parse_known_args(args=[
        '--src=en',
        '--tgt=vi', #en
        '--vocab_prefix=' + vocab_file,
        '--ckpt=' + check_point,
        ])

    return main(infer_data, FLAGS)

def preload_hparams(out_dir, check_point, vocab_file):
    global preloaded_hparams
    ...
    preloaded_hparams = create_hparams(flags)
    preloaded_hparams = create_or_load_hparams(out_dir, preloaded_hparams, flags.hparams_path)

    inference.preload_infer_model(preloaded_hparams, check_point)